...
public class PercentageViewBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public PercentageViewBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);

        // setting values
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SimpleViewBehavior);
        mDependViewId = a.getResourceId(R.styleable.SimpleViewBehavior_svb_dependsOn, 0);
        mDependType = a.getInt(R.styleable.SimpleViewBehavior_svb_dependType, DEPEND_TYPE_WIDTH);
        mDependTarget = a.getDimensionPixelOffset(R.styleable.SimpleViewBehavior_svb_dependTarget, UNSPECIFIED_INT);
        a.recycle();
    }

    public PercentageViewBehavior(Builder builder) {
